...
class PerformanceService {
  ...
  List<String> getPerformanceRecommendations() {
    ...
    if (summary.averageNetworkTime > 3000) {
      recommendations.add(
        'Network performance is slow. Consider implementing offline caching.',
      );
    }

    if (summary.averageFrameTime > 16) {
      recommendations.add(
        'UI rendering is slow. Optimize widget rebuilds and use ListView.builder.',
      );
    }

    if (summary.memoryUsagePercent > 80) {
      recommendations.add(
        'High memory usage detected. Implement proper cleanup and dispose resources.',
      );
    }
    ...
    if (summary.cacheHitRate < 0.6) {
      recommendations.add(
        'Low cache hit rate. Optimize caching strategy and TTL values.',
      );
    }
...
class PerformanceSummary {
  ...
  double get averageNetworkTime {
    ...
    final totalTime = networkOperations.fold(
      0,
  ...
  double get averageFrameTime {
    ...
    final totalTime = uiRenderEvents.fold(
      0,
      (sum, event) => sum + event.frameTimeMs,
    );
