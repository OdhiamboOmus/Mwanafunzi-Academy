...
class PerformanceMonitor {
  ...
  List<String> getRecommendations() {
    ...
    if (summary.cacheHitRate < 0.6) {
      recommendations.add(
        'Low cache hit rate (${(summary.cacheHitRate * 100).toStringAsFixed(1)}%). Consider increasing TTL values.',
      );
    }
    ...
    if (summary.averageReadTime > 100) {
      recommendations.add(
        'High read time (${summary.averageReadTime.toStringAsFixed(1)}ms). Optimize data access patterns.',
      );
    }
    ...
    if (summary.averageNetworkTime > 3000) {
      recommendations.add(
        'Slow network (${summary.averageNetworkTime.toStringAsFixed(1)}ms). Implement offline caching.',
      );
    }
    ...
    if (summary.errorRate > 0.1) {
      recommendations.add(
        'High error rate (${(summary.errorRate * 100).toStringAsFixed(1)}%). Review error handling.',
      );
    }
    ...
    if (summary.totalDownloads > 50) {
      recommendations.add(
        'Frequent downloads (${summary.totalDownloads}). Consider aggressive caching.',
      );
    }
  ...
  void _checkPerformanceHealth() {
    if (isPerformanceDegraded()) {
      if (kDebugMode) {
        ...
        debugPrint(
          '  Average read time: ${summary.averageReadTime.toStringAsFixed(1)}ms',
        );
        debugPrint(
          '  Average network time: ${summary.averageNetworkTime.toStringAsFixed(1)}ms',
        );
        debugPrint(
          '  Error rate: ${(summary.errorRate * 100).toStringAsFixed(1)}%',
        );
      }
    }
  }
...
class PerformanceMetrics {
  ...
  void recordDownload(String contentType, int sizeBytes) {
    ...
    _downloadSizes[contentType] =
        (_downloadSizes[contentType] ?? 0) + sizeBytes;
  }
  ...
  PerformanceSummary getSummary() {
    return PerformanceSummary(
      ...
      totalCacheMisses: _cacheMisses.values.fold(
        0,
        (sum, count) => sum + count,
      ),
      ...
      totalDownloads: _downloadCounts.values.fold(
        0,
        (sum, count) => sum + count,
      ),
