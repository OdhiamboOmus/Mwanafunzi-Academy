...
class TelemetryService {
  ...
  DownloadFrequencyAnalysis getDownloadFrequencyAnalysis() {
    return DownloadFrequencyAnalysis.fromCounts(_collector._downloadFrequencies);
  }
  ...
  List<String> getPerformanceRecommendations() {
    ...
    if (metrics.hitRate < 0.5) {
      recommendations.add('Low cache hit rate (${(metrics.hitRate * 100).toStringAsFixed(1)}%). Consider increasing TTL values for frequently accessed content.');
    } else if (metrics.hitRate > 0.9) {
      recommendations.add('Excellent cache hit rate (${(metrics.hitRate * 100).toStringAsFixed(1)}%). Current caching strategy is working well.');
    }
    
    // Read time recommendations
    if (metrics.averageReadTime > 100) {
      recommendations.add('High average read time (${metrics.averageReadTime.toStringAsFixed(1)}ms). Consider optimizing data structures or storage access patterns.');
    }
    
    // Write time recommendations
    if (metrics.averageWriteTime > 200) {
      recommendations.add('High average write time (${metrics.averageWriteTime.toStringAsFixed(1)}ms). Consider batching writes or optimizing serialization.');
    }
    ...
    if (downloadAnalysis.totalDownloads > 100) {
      recommendations.add('High download frequency (${downloadAnalysis.totalDownloads} downloads). Consider implementing more aggressive caching.');
    }
  ...
  void _generateTelemetryReport() {
    if (kDebugMode) {
      ...
      debugPrint('  Cache Hit Rate: ${(metrics.hitRate * 100).toStringAsFixed(1)}%');
      debugPrint('  Total Reads: ${metrics.totalReads}');
      debugPrint('  Total Writes: ${metrics.totalWrites}');
      debugPrint('  Average Read Time: ${metrics.averageReadTime.toStringAsFixed(1)}ms');
      debugPrint('  Average Write Time: ${metrics.averageWriteTime.toStringAsFixed(1)}ms');
